...
step :: [Char] -> [Char] -> [Int] -> Int -> IO [Int]
step _ [] memory _ = return memory
step previousProgram nextProgram memory pointer = do
    ...
    case instruction of
        '>' -> if pointer == length memory - 1
            then step (previousProgram ++ [instruction]) (tail nextProgram) (memory ++ [0]) (length memory)
